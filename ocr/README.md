...
The repository contains selected `*.traineddata` files in `ocr/tessdata/`. The
build copies them to `tessdata/` next to the built `ocr` module. At runtime the
extension uses that directory as Tesseract's data path.
